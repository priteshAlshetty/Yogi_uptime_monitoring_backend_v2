...
const { getDailyData, getWeeklyData, getMonthlyData, getYearlyData, getWorkHourByMonth, setWorkHourByDate } = require('../controllers/controller.allData');
...
//API to get work hours of month 
router.get("/getworkhoursbymonth", async (req, res) => {
    try {
        const month = req.query.month;
        //format YYYY-MM
        if (!month) {
            return res.status(400).json({ errMsg: "Missing required parameter: month" });
        }
        const workHours = await getWorkHourByMonth(month);
        if (!workHours) {
            return res.status(404).json({ errMsg: `No work hours found for month: ${month}` });
        }
        return res.status(200).json(workHours);
    } catch (error) {
        console.error("Error in /getworkhoursbymonth: ", error);
        res.status(500).json({
            error: "Internal server error",
            message: error.message,
            stack: error.stack,
...
router.post("/setworkhoursbydate", async (req, res) => {
    try {
        const { date, hours } = req.body;
        if (!date || hours === undefined || hours === null) {
            return res.status(400).json({ errMsg: "Missing required parameters: date and hours" });
        }
        const result = await setWorkHourByDate(date, hours);
        if (!result) {
            return res.status(404).json({ errMsg: `Failed to set work hours for date: ${date}` });
        }
        return res.status(200).json({ message: `Work hours set successfully for date: ${date}` });
    } catch (error) {
        console.error("Error in /setworkhoursbydate: ", error);
        res.status(500).json({
            error: "Internal server error",
            message: error.message,
            stack: error.stack,
