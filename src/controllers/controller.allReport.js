...
async function createExcelDaily(date) {
  try {
    ...
    worksheet.getRow(3).values = [
      ...
      'shift1_uptime_HH:MM',
      'shift1_downtime HH:MM',
      'shift2_uptime_HH:MM',
      'shift2_downtime HH:MM'
    ];
    worksheet.columns = [
      { key: 'machine_id', width: 30 },
      { key: 'total_uptime_min', width: 20 },
      { key: 'total_uptime_hr', width: 20 },
      { key: 'total_uptime', width: 20 },
      { key: 'total_downtime', width: 30 },
      { key: 'shift1_uptime', width: 30 },
      { key: 'shift1_downtime', width: 30 },
      { key: 'shift2_uptime', width: 30 },
      { key: 'shift2_downtime', width: 30 },
    ...
    if (!uptimeData || uptimeData.length === 0) {
      ...
      uptimeData.forEach(item => {
        worksheet.getRow(rowIndex).values = [
          ...
          item.Uptime_hr,
          item.Total_hr,
          item.Total_Downtime_hr,
          item.Shift1_Uptime_hr,
          item.Shift1_Downtime_hr,
          item.Shift2_Uptime_hr,
          item.Shift2_Downtime_hr
        ];

        // Downtime columns (E,G,I) = Red
        ['A', 'B', 'C'].forEach(col => {
